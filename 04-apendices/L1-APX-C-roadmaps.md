...
>
> **Recursos de aprofundamento por marcos:** marcos técnicos → Apêndice D (critério de seleção de ferramentas e stack); marcos de fundamentação → Apêndice G (papers fundamentais); marcos de contexto de mercado → Apêndice E (como montar sua dieta de informação).
...
- Portfólio ativo com pelo menos três clientes aplicando o método como norma, com case documentado por cada um
...
## Roadmap 7 — Profissional Individual (IC / Individual Contributor)

> Perfil: analista sênior, especialista funcional, product manager ou profissional de qualquer área em empresa que ainda não tem estratégia de IA definida — sem mandato organizacional, sem equipe sob gestão, operando dentro do próprio escopo de trabalho.

### Marco 30 dias
**Tese:** instalar o método no trabalho próprio, sem precisar de mandato.

- Identificar três tarefas repetitivas do dia a dia e aplicar o Método de Decisão para Adotar IA em cada uma
- Aplicar a Engenharia de Prompt Estendida em pelo menos uma tarefa em produção
- Montar golden set pessoal mínimo: dez casos com gabarito para a tarefa escolhida

**Critério de avanço:** uma tarefa instrumentada com golden set v0 e prompt versionado.
...
### Marco 180 dias
**Tese:** tornar-se referência informal no próprio contexto.
...
> Recursos recomendados: APX-E (como montar dieta de informação para quem aprende sem equipe); APX-G (papers de fundamento para quem quer profundidade técnica no próprio ritmo); APX-D (critério D.1 para avaliar ferramentas individualmente).
